...
def get_cache_file_path(cache_name: str) -> Path:
    ...
    cache_home = (
        Path(cache_home_value) if cache_home_value else Path(_home.get_home_dir()) / ".cache"
    )
...
class CacheKeyCodec:
    ...
    @classmethod
    def decode(cls, payload: dict[str, Any]) -> Hashable:
        kind = payload["kind"]
        try:
...
def _decode_primitive_cache_key(payload: dict[str, Any]) -> Hashable:
    value = payload["value"]
    if not isinstance(value, Hashable):
        raise TypeError(f"Cached primitive key {value!r} is not hashable")
    return value
...
def _decode_enum_cache_key(payload: dict[str, Any]) -> Enum:
    ...
    if not isinstance(enum_type, EnumMeta):
        raise TypeError(f"Cached enum owner {enum_type!r} is not an Enum type")
...
CACHE_KEY_DECODERS: dict[str, Callable[[dict[str, Any]], Hashable]] = {
...
class RegistryCacheManager(Generic[T]):
    ...
    def __init__(
        ...
        file_mtimes_getter: Callable[[], dict[str, float]] | None = None,
    ):
        """
    ...
    def load_cache(self) -> dict[Hashable, T] | None:
        """
        ...
        if self.config.check_mtimes and "file_mtimes" in cache_data:
            current_mtimes = (
                self.file_mtimes_getter() if self.file_mtimes_getter is not None else None
            )
    ...
    def _validate_mtimes(
        ...
        *,
        ...
        for file_path, cached_mtime in cached_mtimes.items():
            ...
            current_mtime = (
                current_mtimes[file_path] if current_mtimes is not None else path.stat().st_mtime
            )
...
def deserialize_plugin_class(data: dict[str, Any]) -> type:
    ...
    for owner_name in data["qualname"].split("."):
        if owner_name == "<locals>":
            raise AttributeError("Function-local classes cannot be restored from a registry cache")
        declaration = getattr(declaration, owner_name)
    if not isinstance(declaration, type):
        raise TypeError(f"Cached plugin declaration {data['qualname']!r} is not a class")
    return declaration
    ...
    *,
    ...
    try:
        ...
        if spec.submodule_search_locations:
            package_dirs = [Path(location) for location in spec.submodule_search_locations]
        ...
        for package_dir in package_dirs:
            source_files = package_dir.rglob("*.py") if recursive else package_dir.glob("*.py")
